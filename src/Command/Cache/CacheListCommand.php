...
final class CacheListCommand extends Command
{
    ...
    {
        $this->addOption('json', null, InputOption::VALUE_NONE, 'Emit machine-readable cache metadata.');
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        if (is_dir($root)) {
            ...
            foreach ($entries as $entry) {
                ...
                $repositories[] = [
                    ...
                    'last_used_at' => $lastUsed !== '-' ? $lastUsed : null,
                ];
            }
        }
        ...
        foreach ($repositories as $repository) {
            $output->writeln(sprintf(
                ...
                $repository['repository'],
                $repository['ref'] ?? '-',
                $repository['commit'] !== null ? substr($repository['commit'], 0, 12) : '-',
                $repository['last_used_at'] ?? '-',
            ));
        }
